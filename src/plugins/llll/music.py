...
def _get_stage_info(
    dm, stage_entries, fever_section_no=None, level_key=None, group_type=None
):
    stages = []
    for stage_entry in stage_entries:
        ...
        stage_name = (
            live_stage.get("Name")
            if live_stage
            else stage_entry.get("Name") or f"Stage {live_stage_id}"
        )
        stage_desc = (
            live_stage.get("Description")
            if live_stage
            else stage_entry.get("Description") or ""
        )
        ...
        if live_stage:
            ...
            stage_skill_sets = dm.get_stage_skill_sets(
                live_stage.get("StageSkillSetIds") or []
            )
            if stage_skill_sets:
                for ss in stage_skill_sets:
                    ...
                    for detail in effect_details:
                        effect_parts.append(
                            f"{detail.get('SkillEffectDetailType')}={detail.get('EffectValue')}"
                        )
                    effect_text = " / ".join(effect_parts) if effect_parts else "-"
                    stage_data["effects"].append(
                        {"text": f"区段 {ss['id']}: {effect_text}"}
                    )
...
async def _(args: Message = CommandArg()):
    ...
    for entry in results:
        ...
        if quest_live_stages:
            music_data["quest_live_stages"] = _get_stage_info(
                dm,
                quest_live_stages,
                fever_section_no=fever_section_no,
                level_key="QuestLevel",
                group_type="quest",
            )
        if grade_live_stages:
            music_data["grade_live_stages"] = _get_stage_info(
                dm,
                grade_live_stages,
                fever_section_no=fever_section_no,
                level_key="LivePoint",
        ...
        if grand_prix_stages:
            music_data["grand_prix_stages"] = _get_stage_info(
                dm,
                grand_prix_stages,
                fever_section_no=fever_section_no,
                level_key="QuestLevel",
                group_type="grand_prix",
            )
