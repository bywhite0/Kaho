...
- PIL 渲染画廊总览与缩略图墙（带磁盘缓存），封面可指定，两者均显示图片数与占用体积
...
| 添加画廊别名 \<名称\> \<别名\> | 所有人 | |
| 删除画廊别名 \<别名\> | 超级用户 | |
| 看所有 [画廊] | 所有人 | 无参=总览图；带名=该画廊图片墙；均带图片数与占用体积 |
| 看 \<画廊\> [xN] | 所有人 | 随机抽图，同一批内不重复 |
| 看 \<图片id...\> | 所有人 | 按 id 取图，可一次给多个；`-1` 为最新入库的一张，`-2` 次新 |
| 看 #\<图片id...\> | 所有人 | 显式按 id 取图，不会被同号的纯数字别名遮蔽 |
...
画廊名称与别名共用的限制：长度 ≤ 32；不能含空白字符（命令按空白分割参数）、控制字符、`<>:"/\|?*`；不能以 `.` 结尾或撞上 Windows 保留设备名（CON/NUL/COM1…）。

唯一的差异是纯数字：**画廊名不能是纯数字**（它兼作目录名与主标识，且没有「删掉即撤销」这种手段），**别名可以是纯数字**。纯数字别名会让 `看 <该数字>` 命中画廊而非同号图片，此时用 `看 #<图片id>` 精确取图；`删除图片` / `图片信息` / `打标签` / `设置封面` 都不查画廊名，不受遮蔽影响。别名随时可删，这种遮蔽是可撤销的。
...
- 校验画廊名用 `names.validate_gallery_name`，校验别名用 `names.validate_gallery_alias`（二者仅在「能否为纯数字」上分叉）；识别图片 id 用 `names.INTEGER_PATTERN`——画廊名与 id 形式互斥，所以裸数字 token 永远指向图片，纯数字别名的遮蔽只发生在 `看` 的单 token 通路上
- 解析 `#<图片id>` 用 `names.parse_hash_picture_ids`：它要求去掉 `#` 后每个 token 都是整数，否则返回空列表退回画廊名解析——这样以 `#` 开头的画廊名不会被抢走
