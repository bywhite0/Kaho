...
| 设置封面 \<画廊\> \<图片id\|清除\> | 超管/群管 | 指定总览图使用的封面 |
| 画廊查重 \<画廊\> [rehash] | 超管/群管 | 补扫库内已存在的重复图片 |
...
管理命令分两档。**超管/群管**档对超级用户、以及群聊中的群主/群管理员开放；其余管理命令仍是超级用户专属：

| 档位 | 命令 | 为什么这样分 |
|---|---|---|
| 超管 + 群管 | 添加画廊、删除画廊别名、删除图片、设置封面、画廊查重 | 影响可控或有回收站兜底 |
| 仅超管 | 删除画廊、画廊模式、导出画廊、画廊权限 | 见下 |

保留在超管的四个各有原因：

- **画廊权限**——下放等于交出整个权限系统，群管可自授读写、改默认策略掀掉白名单、封禁别的群
- **导出画廊**——把整库打包传到执行者所在群，是唯一的数据外流面且撤不回
- **删除画廊**——连目录带图整体移除，全局影响面最大
- **画廊模式**——它是运营方锁定画廊的手段，交出去这个功能就形同虚设

群职位取自消息事件的 `sender.role`，不额外请求协议端。**私聊里群管权限不生效**（私聊事件无群职位可判），超管不受影响；协议端不上报 `role` 时判定失败并退回仅超管（fail-closed）。

三条边界要清楚：

1. **群管不越过画廊模式**——`只读` 画廊对群管仍然只读，`off` 画廊对群管仍然不可见（回复一律伪装成"未找到"，不泄露其存在）
2. **黑白名单优先于群职位**——所在群被设为 `只读`/`禁用` 时，群管一条管理命令都用不了
3. **画廊是全局资源**——没有"归属群"概念，A 群群管删掉的图片对所有群一起消失。这是有意接受的：授权模型信任群职位本身，真出问题用 `画廊权限` 把该群降级

已知的残余风险：群管可通过 `添加画廊` 刷垃圾画廊占用磁盘（未设配额）。
...
先在本进程跑纯逻辑用例（名称校验、权限策略、查重指纹），再以子进程方式跑需要 NoneBot 运行时的状态用例、授权用例（`test_permission.py`：群职位判定与可见性/只读门）与插件加载冒烟。
