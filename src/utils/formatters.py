...
def parse_intro(intro_text):
    data = {}
    labels = ['誕生日', '身長', '趣味', '特技', '好きな食べ物', '好きな言葉', '好きな教科', '好きな動物']
    label_map = {'誕生日': 'Birthday', '身長': 'Height', '趣味': 'Hobbies', '特技': 'Special Skills', '好きな食べ物': 'Favorite Food', '好きな言葉': 'Favorite Word', '好きな教科': 'Favorite Subject', '好きな動物': 'Favorite Animal'}
    lines = intro_text.split('\n')
    for line in lines:
        segments = [s for s in re.split(r'[\s　]+', line.strip()) if s]
        for i, seg in enumerate(segments):
            if seg in labels and i + 1 < len(segments) and segments[i + 1] not in labels:
                data[label_map[seg]] = segments[i + 1]
    return data


def print_merged_skill(dm, skill_data, title_prefix="", is_token=False, cost_str="", show_type=True):
    if not skill_data:
        return ""
